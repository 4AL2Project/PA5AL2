...
export class ConsoleNotificationService implements INotificationService {
  ...
  async orderConfirmed(payload: OrderNotificationPayload): Promise<void> {
    this.logger.log(
      `[ORDER_CONFIRMED] → ${payload.customer_email} | order=${payload.order_id} | qr=${payload.qr_code}`
    );
  }

  async orderReady(payload: OrderNotificationPayload): Promise<void> {
    this.logger.log(
      `[ORDER_READY] → ${payload.customer_email} | order=${payload.order_id} | ${payload.product_name} @ ${payload.pharmacy_name}`
    );
  }
  ...
  async newOrderForPrep(payload: OrderNotificationPayload): Promise<void> {
    this.logger.log(
      `[NEW_ORDER_FOR_PREP] pharmacy=${payload.pharmacy_name} | order=${payload.order_id} | ${payload.quantity}× ${payload.product_name}`
    );
  }
}
