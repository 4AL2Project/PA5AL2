...
function PreparateurDialog({
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = target
        ? `/api/admin/pharmacies/${pharmacyId}/preparateurs/${target.user_id}`
        : `/api/admin/pharmacies/${pharmacyId}/preparateurs`;
  ...
  const onDelete = async () => {
    ...
    try {
      const res = await fetch(
        `/api/admin/pharmacies/${pharmacyId}/preparateurs/${deleteTarget.user_id}`,
        { method: 'DELETE' }
      );
      if (res.ok) {
      ...
      {preparateurs.length === 0 ? (
        <div className="rounded-xl border bg-card p-10 text-center">
          ...
          <p className="mx-auto mt-1 max-w-sm text-xs text-muted-foreground">
            Ajoutez un préparateur de commande pour cette officine. Il se
            connectera via un lien magique envoyé à son email.
          </p>
        </div>
      ) : (
