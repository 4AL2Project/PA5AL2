          ...
          <div className="space-y-2">
            {alerts.blocked_count > 0 && (
              <div className="rounded-xl border border-red-200 bg-red-50 p-4">
                <div className="flex items-center gap-2 text-sm font-medium text-red-800">
                  ...
                  {alerts.blocked_count} don
                  {alerts.blocked_count > 1 ? 's' : ''} bloqué
                  {alerts.blocked_count > 1 ? 's' : ''} depuis plus de 5 jours
                </div>
                  ...
                  <ul className="mt-3 space-y-1.5">
                    {alerts.blocked_donations.slice(0, 5).map((d) => (
                      ...
                      >
                        ...
                        >
                          <Link href={`/admin/dons/${d.donation_id}`}>
                            Voir
                          </Link>
                        </Button>
                      </li>
            ...
            {alerts.expired_proposals > 0 && (
              <div className="flex items-center gap-2 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
                ...
                {alerts.expired_proposals} proposition
                {alerts.expired_proposals > 1 ? 's' : ''} expirée
                {alerts.expired_proposals > 1 ? 's' : ''} non relancée
                {alerts.expired_proposals > 1 ? 's' : ''}
              </div>
            )}
            ...
            {alerts.missed_pickups > 0 && (
              <div className="flex items-center gap-2 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
                ...
                {alerts.missed_pickups} retrait
                {alerts.missed_pickups > 1 ? 's' : ''} manqué
                {alerts.missed_pickups > 1 ? 's' : ''}
              </div>
            )}
            ...
            {alerts.low_reliability_assos.length > 0 && (
              <div className="rounded-xl border border-amber-200 bg-amber-50 p-4">
                <div className="flex items-center gap-2 text-sm font-medium text-amber-800">
                  ...
                  {alerts.low_reliability_assos.length} association
                  {alerts.low_reliability_assos.length > 1 ? 's' : ''} avec
                  fiabilité &lt; 50 %
                </div>
...
function KpiCard({
  ...
  return (
    <div className="rounded-xl border bg-card p-4">
      ...
      <p
        className={`mt-1 text-2xl font-bold tabular-nums ${green ? 'text-emerald-700' : ''}`}
      >
        {value}
      </p>
    </div>
